...
module.exports.signUp = async (req,res) => {
   ...
   const {pseudo, email, trustedEmail, password, secretAnswer, secretQuestion} = req.body;
   if (secretQuestion && !secretAnswer) {
    return res.status(400).json({error: 'secretAnswer'});
          
  }
   try {
    const user = await UserModel.create({pseudo, email, trustedEmail, password, secretAnswer, secretQuestion});
